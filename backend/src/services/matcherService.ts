...
export function computeItemMatchScore(itemA: any, itemB: any): MatchScoreResult {
  let score = 0;
 ...
 */
export async function runMatchmakerForItem(
  newItem: any,
  io?: SocketIOServer
): Promise<any[]> {
  ...
  for (const candidate of potentialMatches) {
    ...
    if (match.totalScore >= 45) {
      ...
      const notificationRecord = await prisma.matchNotification.create({
        data: {
          userId: notifyUserId,
