...
import { sanitizeAndSaveImage } from '../services/imageService';
import { runMatchmakerForItem, calculateDistanceInMeters } from '../services/matcherService';
...
export async function createItem(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (req.file) {
      const sanitizedImage = await sanitizeAndSaveImage(req.file.buffer, req.file.originalname);
    ...
    const handoverCode = `YEN-${crypto.randomBytes(3).toString('hex').toUpperCase()}`;
    ...
    const matches = await runMatchmakerForItem(item, io);
...
export async function getItems(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (lat && lng && radiusMeters) {
      ...
      const maxRadius = parseFloat(radiusMeters as string);

      if (!isNaN(centerLat) && !isNaN(centerLng) && !isNaN(maxRadius)) {
        filteredItems = items.filter((item) => {
          const dist = calculateDistanceInMeters(centerLat, centerLng, item.latitude, item.longitude);
...
export async function updateItemStatus(req: AuthenticatedRequest, res: Response) {
  try {
    if (!req.user) {
...
export async function verifyHandoverScan(req: AuthenticatedRequest, res: Response) {
  try {
    if (!req.user) {
    ...
    const returnerUser = await prisma.user.update({
      where: { id: req.user.id },
      data: {
        points: { increment: 100 },
        itemsReturned: { increment: 1 },
