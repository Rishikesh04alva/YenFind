...
async function submitClaim(req, res) {
    try {
        ...
        const existingClaim = await db_1.prisma.claim.findFirst({
            where: {
                itemId,
        ...
        const claim = await db_1.prisma.claim.create({
            data: {
                itemId,
                claimantId: req.user.id,
...
async function reviewClaim(req, res) {
    try {
        ...
        const claim = await db_1.prisma.claim.findUnique({
            where: { id: claimId },
        ...
        const updatedClaim = await db_1.prisma.claim.update({
            where: { id: claimId },
            data: { status },
        });
...
async function sendItemMessage(req, res) {
    try {
        if (!req.user) {
            return res.status(401).json({ success: false, error: 'Unauthorized.' });
        }
